...
所有权方式检查发生在编译器，所以对运行时不影响，故而不影响程序性能。可以理解成与 C++ 不同的是，就是 Rust 编译器在编译时自动根据它提供的所有权机制，帮你的代码实现了管理内存分配和释放的部分。
...
上文中说到，基础数据类型中字符串字面量是写入编码里面的，长度不可变，如果需要使用可变的字符串呢？Rust 标准库提供了 `String` 类型，
